	...
	window.onscroll = () => {
		if (window.innerHeight + window.scrollY >= document.body.offsetHeight - 5) {
			...
			if (flag) {
				...
				const request = new XMLHttpRequest();
				request.open("POST", "/worldfeed");
				const data = new FormData();
				data.append("csrfmiddlewaretoken", document.querySelector("input[name='csrfmiddlewaretoken']").value);
				data.append("contents", JSON.stringify(posted_content));
				request.send(data);
				request.onreadystatechange = () => {
					if (request.status === 200 && request.readyState === 4) {
								...
								if (JSON.parse(request.response)["content_data"][i]) {
									...
									content_HTML_build = `<div class="row content-owner-details" id="content-owner-info-5">
																	...
																	<div
																		...
																		<div class="dropleft">
																			<button class="btn btn-sm dropdown-toggle little-dropdown-dots" data-toggle="dropdown" aria-expanded="false" type="button">...</button>
																			<div class="dropdown-menu" role="menu">
																				<a class="dropdown-item dropdown-menu-options copy-link-btn" role="presentation" data-content="`+JSON.parse(request.response)["content_data"][i]["pk"]+`">
																					<i class="fas fa-link" ></i> Copy Link
																				</a>
																				<a class="dropdown-item dropdown-menu-options recommend-btn" role="presentation" data-content="`+JSON.parse(request.response)["content_data"][i]["pk"]+`">
																					`+recommended_status+`
																				</a>
																			</div>
																		</div>
															...
															<div class="row" id="content-details-5">
																<div class="col">
																	...
																	<div class="row">
																		<div class="col-xl-4">
																			<div class="btn-group" role="group"><button class="btn content-vote-btn up-vote-btn" type="button" data-content="`+JSON.parse(request.response)["content_data"][i]["pk"]+`"><i class="fa fa-chevron-up"></i></button><button class="btn btn-lg content-vote-btn down-vote-btn" type="button" data-content="`+JSON.parse(request.response)["content_data"][i]["pk"]+`"><i class="fas fa-angle-down"></i></button></div>
																		</div>
									...
									if (content_HTML_build) {
										...
										html_o.className = "col-12 col-sm-12 col-md-5 col-lg-5 col-xl-5 offset-xl-0 col-content-box";
